...
const TaskBoard = ({ refreshTrigger }) => {
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-4 w-full min-h-[500px] pb-4">
      {Object.entries(COLUMNS).map(([status, bgClass]) => (
        <div key={status} className="flex-1 relative p-[1px] rounded-[30px] bg-gradient-to-br from-white to-gray-500">
          <div className={clsx("h-full w-full p-4 rounded-[29px]", bgClass)}>
            <h3 className="font-bold text-lg mb-4 text-gray-700 dark:text-gray-200 px-2">{status}</h3>
